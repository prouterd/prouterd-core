...
process inference
 ...
 block train
  interface docker alpine
  command `sh -c "echo MODEL > /prouter/artifacts/model.pkl && echo '{\"acc\":0.92}' > /prouter/artifacts/metrics.json && echo '{\"trained\":true}'"`
  produces model.pkl
 ...
 block deploy
  interface docker alpine
  command `sh -c "cat $PROUTER_INPUT_MODEL && cat $PROUTER_INPUT_METRICS && echo '{\"deployed\":true}'"`
