...
# We don't require the `pg` gem in test — the caller is structured so it
# fails cleanly when pg is missing. For tests that exercise a real query
# path we stub the PG interface entirely.
RSpec.describe Prouterd::Iface::PostgresCaller do
  let(:caller) { described_class.new }

  def fake_iface(dsn: "postgres://x@h/db", statement_timeout: nil)
    type_fields = { "dsn" => dsn }
    type_fields["statement-timeout"] = statement_timeout if statement_timeout
    Struct.new(:name, :type, :type_fields).new("warehouse", "postgres", type_fields)
  end
  ...
  describe "with pg available (stubbed)" do
    before do
      ...
      # Stub the PG module + connection so we don't need libpq.
    ...
    def fake_result(rows: [], fields: [], row_count: 0)
      double("PG::Result").tap do |d|
        # The caller iterates with `result.map`, so we need an iterator-shaped
        # double. `each` yielding the rows is enough.
    ...
    it "returns rows + row_count + fields on a successful query" do
      ...
      result = caller.call(
        iface: fake_iface,
        call_fields: { "query" => "SELECT id, status FROM tickets" }
      )
    ...
    it "binds params via exec_params when present" do
      ...
      result = caller.call(
        iface: fake_iface,
        call_fields: { "query" => "SELECT id FROM tickets WHERE key = $1", "params" => "JIRA-42" }
      )
      expect(result.exit_code).to eq(0)
      expect(result.output_json["row_count"]).to eq(0)
    end
    ...
    it "splits multiple comma-separated params" do
      ...
      caller.call(
        iface: fake_iface,
        call_fields: { "query" => "INSERT ... VALUES ($1, $2, $3)", "params" => "a, b, c" }
      )
    end
    ...
    it "applies statement-timeout via SET LOCAL when present" do
      ...
      caller.call(
        iface: fake_iface(statement_timeout: "5000"),
        call_fields: { "query" => "SELECT 1" }
      )
    end
    ...
    it "rolls back and reports sql_error on a PG::Error" do
      ...
      result = caller.call(
        iface: fake_iface,
        call_fields: { "query" => "SELECT bogus" }
      )
